...
export default function AnimatedSwitch(props) {
    const getAnimationStyle = (p) => {
        const isSubroute = routes.some(r => r.subroutes && r.subroutes.some(sr => sr.path === p));
        ...
        <SwitchTransition>
            ...
            >
                <Routes location={props.location}>
                    ...
                    {routes.map(r => r.subroutes && r.subroutes.map(sr => (
                        <Route key={sr.path} path={sr.path} element={
                            <Subcard
                                label={sr.label}
                                backTo={r.path}
                                image={sr.image}
                    ...
                    <Route path={pageNotFound.path} element={
                        <Card
